...
const generateSubtractionProblem = () => {
  const num1 = Math.floor(Math.random() * 20) + 1 // Number between 1 and 20
  const num2 = Math.floor(Math.random() * num1) + 1 // Number between 1 and num1
  return { num1, num2 }
}

const { state, checkAnswer } = useGame((a, b) => a - b, generateSubtractionProblem)
